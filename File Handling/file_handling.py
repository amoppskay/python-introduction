...
# imports
from os.path import dirname, join # imports from python module, OS, dirname, join
import os

# Access the file in the read mode
print("\n# Access the file in the read mode")
# the path is depends on the OS and depends on the folder, IDE, vscode requires the full path then you add extra \
# file_data = open('sample.txt', 'r')
file_data = open('C:\\Users\\amopp\OneDrive\\Desktop\\python-introduction\\File Handling\\sample.txt', 'r') # an open bug from the vs code side https://stackoverflow.com/questions/51006989/visual-studio-code-filenotfounderror-errno-2-no-such-file-or-directory
print(file_data.read())
# a file called sample.txt is present on the path, we used the open fxn to call the file and r is read mode to be able to read it
# putting it \\ makes it compatible with vscode to be able to let us open it from the path properly

# # Lets try to fix the folder path issue, read the file using automated path
print("\n# Lets try to fix the folder path issue, read the file using automated path")
current_dir = dirname(__file__) # we use a fxn "dirname" this will add the file path and called it a variable named current_dir
file_path = join(current_dir, ".\sample.txt") # we use a fxn "join" join the sample.txt to current_dir instead of using the full path
file_data= open(file_path, 'r') # calls the file path for us to able to open and read it
print(file_data.read())

# # # read first we characters of the file
print("\n# read first we characters of the file") #all below helps us avoid calling the absolute/full file path as above
current_dir = dirname(__file__)
file_path = join(current_dir, "./sample.txt")
file_data= open(file_path, 'r')
print(file_data.read(5)) # only 1st 5 xters will be opened and read
...
print(file_data.readline(), end="") #readline is to read 1 line after another, use r to read the whole file
